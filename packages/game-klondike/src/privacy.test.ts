...
describe('Klondike privacy boundary', () => {
  it('hides stock order and tableau-down identities while preserving public faces and counts', () => {
    ...
    const publicIds = new Set(session.state.tableau.flatMap((column) => column.up));
    const hiddenIds = [
      ...session.state.stock,
  ...
  it('emits opaque setup flights for 21 down cards and public ids only for seven upcards', () => {
  ...
  it('does not advertise Veil, bots, multiplayer seats, a seed, or a daily key in state', () => {
    ...
    expect(klondikeGame.veil).toBeUndefined();
    expect(klondikeGame.bots).toEqual([]);
    expect(session.seats).toBe(1);
    expect(session.state).not.toHaveProperty('seed');
    expect(session.state).not.toHaveProperty('dailyKey');
    expect(klondikeGame.flow.legalMovesFor?.(session.state, session.phase, 1)).toEqual([]);
  });
});
