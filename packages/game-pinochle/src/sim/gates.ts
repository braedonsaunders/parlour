...
/**
 * Machine-checked balance gates for partnership Pinochle:
...
const gameDef = () => createPinochleDef();
const quickConfig = () => pinochleConfig.resolve({ target: 100 });
// Dealer starts at seat 0 every game, so hand 1 always gives team 1 the first
// bid — a real, legitimate edge (first crack at the auction), not a bug. A
// longer match rotates the dealer through more hands and averages it out, so
// the symmetry gate runs to a higher target than the other two gates.
const symmetryConfig = () => pinochleConfig.resolve({ target: 500 });
