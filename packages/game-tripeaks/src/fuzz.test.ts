...
import { leftoverOf, legalMovesFor, tripeaksGame } from './game';
...
describe('TriPeaks seed and move fuzz', () => {
  it('conserves all 52 unique cards across 10,000 seeded deals', () => {
    ...
    for (let seed = 0; seed < 10_000; seed++) {
      ...
      expect(leftoverOf(openSession(seed).state)).toBe(18);
    }
  });

  it('makes every enumerated move apply and ends when nothing legal remains', () => {
    for (let seed = 0; seed < 400; seed++) {
      let session = openSession(seed * 97 + 11, {
        wrap: seed % 2 === 0,
        recycle: seed % 3 === 0,
      });
      for (let step = 0; step < 80 && session.status === 'playing'; step++) {
        ...
        expect(leftoverOf(session.state)).toBe(
          session.state.tableau.filter((card) => card !== null).length,
        );
      }
