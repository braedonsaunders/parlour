...
describe('TriPeaks privacy boundary', () => {
  ...
  it('emits public setup flights for the eighteen peak cards and the opening hole', () => {
    ...
    expect(payloads.filter((payload) => payload.to === 'hole')).toHaveLength(1);
  ...
  it('does not advertise Veil, bots, multiplayer seats, a seed, or a daily key in state', () => {
    ...
    expect(tripeaksGame.veil).toBeUndefined();
    expect(tripeaksGame.bots).toEqual([]);
    expect(session.seats).toBe(1);
    expect(session.state).not.toHaveProperty('seed');
    expect(session.state).not.toHaveProperty('dailyKey');
    expect(tripeaksGame.flow.legalMovesFor?.(session.state, session.phase, 1)).toEqual([]);
  });
});
