...
/**
 * Headless Hearts bot simulator: `pnpm --filter @parlour/game-hearts sim -- --games 2000`
 * plays mixed bot tables and enforces the balance gates. Exit 0 = gates pass.
 */
...
console.log('');
console.log('gate 1 — skill ladder in mixed tables (Sharp/Careful/Harmless)');
for (const row of report.ladder.rows) {
  console.log(`  ${row.key.padEnd(10)} ${(row.winRate * 100).toFixed(1)}% (${row.games} games)`);
}
console.log(`  — ${report.ladder.passes ? 'PASS' : 'FAIL'}`);
