...
describe('FreeCell card rules', () => {
  ...
  it('builds tableau down in alternating colors and admits any card to empties', () => {
    ...
    expect(canPlaceOnTableau('C12', null)).toBe(true);
    expect(canPlaceOnTableau('S1', null)).toBe(true);
  ...
  it('uses the Microsoft supermove formula', () => {
    expect(maxMovable(0, 0)).toBe(1);
    expect(maxMovable(1, 0)).toBe(2);
    expect(maxMovable(0, 1)).toBe(2);
    expect(maxMovable(3, 2)).toBe(16);
  });
});
