...
/**
 * Headless Blitz bot simulator (spec §4.1/§9): `pnpm sim -- --games 10000`
 * plays bots vs bots and enforces the balance gates. Exit code 0 = gates pass.
 */
...
console.log(
  `gates: hard ≥ ${(DEFAULT_THRESHOLDS.headToHeadMin * 100).toFixed(0)}% vs easy · persona band ${(DEFAULT_THRESHOLDS.personaBandMin * 100).toFixed(0)}–${(DEFAULT_THRESHOLDS.personaBandMax * 100).toFixed(0)}%`,
);
...
for (const row of report.personas.rows) {
  const flag =
    row.winRate < report.thresholds.personaBandMin || row.winRate > report.thresholds.personaBandMax
      ...
      : '';
  console.log(
    `  ${row.key.padEnd(14)} ${(row.winRate * 100).toFixed(1)}% (${row.games} games)${flag}`,
  );
}
console.log(`  — ${report.personas.passes ? 'PASS' : 'FAIL'}`);
if (report.stalls > 0) {
  console.log(`  (${report.stalls} abandoned games)`);
}
