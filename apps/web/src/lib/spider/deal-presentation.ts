...
import { COLUMN_LENGTHS, TABLEAU_COLUMNS } from '@parlour/game-spider';

type DealCue = { column: number; landsAt: number };

export interface SpiderDealPresentation {
  sequence: boolean;
  dealing: boolean;
  visibleByColumn: readonly number[];
}

const OPENING_DEAL = COLUMN_LENGTHS.reduce((sum, length) => sum + length, 0);
...
export function useSpiderDealPresentation(
  fx: readonly FxEvent[],
  fxKey: string | number,
  reduced: boolean,
): SpiderDealPresentation {
  const plan = useMemo(() => planDeal(fx), [fx]);
  const isOpening = plan.length === OPENING_DEAL;
      ...
      window.setTimeout(() => {
        setProgress((current) => {
          ...
          const next = counts.map((count, column) =>
            column === cue.column ? Math.min(COLUMN_LENGTHS[column] ?? 0, count + 1) : count,
          );
          return {
            key: fxKey,
            counts: next,
