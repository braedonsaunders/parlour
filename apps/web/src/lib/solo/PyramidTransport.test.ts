...
describe('PyramidTransport', () => {
  ...
  it('restarts the same seed with the original setup choreography', () => {
    ...
    expect(restarted.fx.filter((event) => event.kind === 'card.fly')).toHaveLength(PYRAMID_SIZE);
  });
});
