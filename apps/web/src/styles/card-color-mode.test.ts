...
describe('global app color modes', () => {
  it('keeps an identity layer in original and applies one richer pass to the full app', () => {
    ...
    expect(globals).toMatch(
      /--unplayable-filter:\s*grayscale\(0\.42\) saturate\(0\.5\) brightness\(0\.76\)/,
    );
    expect(globals).toMatch(/--unplayable-opacity:\s*1/);
    expect(globals).toMatch(
      /html\[data-color-mode='richer'\]\s*\{[^}]*filter:\s*saturate\(1\.26\)[^}]*--unplayable-filter:\s*grayscale\(0\.2\) saturate\(0\.6\) brightness\(0\.78\)[^}]*--unplayable-opacity:\s*1/s,
    );
