...
import { dailyResultFor, dailyStreak, useTripeaksStatsStore } from './tripeaksStats';

beforeEach(() => {
  useTripeaksStatsStore.getState().reset();
});

describe('TriPeaks local stats', () => {
  it('deduplicates starts and finished deals by run id', () => {
    const stats = useTripeaksStatsStore.getState();
    ...
    expect(useTripeaksStatsStore.getState()).toMatchObject({
      dealsStarted: 1,
      holesCompleted: 1,
      clears: 0,
      bestScore: 4,
  ...
  it('keeps the lowest leftover as the daily best and counts a clear', () => {
    ...
    expect(dailyResultFor(useTripeaksStatsStore.getState().dailyResults, '2026-08-24')).toEqual({
      key: '2026-08-24',
      bestScore: 0,
      bestMoves: 28,
      bestTimeMs: 70_000,
