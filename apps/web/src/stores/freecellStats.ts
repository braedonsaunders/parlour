...
export interface FreecellDailyResult {
  key: string;
  bestMoves: number;
  bestTimeMs: number;
...
export interface FreecellStatsState {
  dealsStarted: number;
  wins: number;
  bestMoves: number | null;
  bestTimeMs: number | null;
  dailyResults: readonly FreecellDailyResult[];
...
const EMPTY = {
  ...
  bestMoves: null,
  bestTimeMs: null,
  dailyResults: [] as readonly FreecellDailyResult[],
  startedRunIds: [] as readonly string[],
  wonRunIds: [] as readonly string[],
};

export const useFreecellStatsStore = create<FreecellStatsState>()(
  persist(
    (set) => ({
      ...EMPTY,
      recordStart: (runId) =>
    ...
    {
      name: FREECELL_STATS_STORAGE_KEY,
      version: 1,
      partialize: (state) => ({
...
function upsertDaily(
  current: readonly FreecellDailyResult[],
  result: FreecellDailyResult,
): FreecellDailyResult[] {
...
export function dailyResultFor(
  results: readonly FreecellDailyResult[],
  key: string,
): FreecellDailyResult | null {
  return results.find((result) => result.key === key) ?? null;
}

/** Includes today when complete; otherwise a current streak may end yesterday. */
export function dailyStreak(results: readonly FreecellDailyResult[], todayKey: string): number {
