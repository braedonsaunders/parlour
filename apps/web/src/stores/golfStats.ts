...
export interface GolfDailyResult {
  key: string;
  bestScore: number;
  bestMoves: number;
  bestTimeMs: number;
...
export interface GolfStatsState {
  dealsStarted: number;
  holesCompleted: number;
  clears: number;
  bestScore: number | null;
  bestTimeMs: number | null;
  dailyResults: readonly GolfDailyResult[];
...
const EMPTY = {
  ...
  bestScore: null,
  bestTimeMs: null,
  dailyResults: [] as readonly GolfDailyResult[],
  startedRunIds: [] as readonly string[],
  finishedRunIds: [] as readonly string[],
};

export const useGolfStatsStore = create<GolfStatsState>()(
  persist(
    (set) => ({
      ...EMPTY,
      recordStart: (runId) =>
...
function upsertDaily(
  current: readonly GolfDailyResult[],
  result: GolfDailyResult,
): GolfDailyResult[] {
...
function betterMoves(prior: GolfDailyResult, result: GolfDailyResult): number {
...
function betterTime(prior: GolfDailyResult, result: GolfDailyResult): number {
...
export function dailyResultFor(
  results: readonly GolfDailyResult[],
  key: string,
): GolfDailyResult | null {
  return results.find((result) => result.key === key) ?? null;
}

/** Includes today when complete; otherwise a current streak may end yesterday. */
export function dailyStreak(results: readonly GolfDailyResult[], todayKey: string): number {
