...
export interface KlondikeDailyResult {
  key: string;
  bestMoves: number;
  bestTimeMs: number;
...
export interface KlondikeStatsState {
  dealsStarted: number;
  wins: number;
  bestMoves: number | null;
  bestTimeMs: number | null;
  dailyResults: readonly KlondikeDailyResult[];
...
const EMPTY = {
  ...
  bestMoves: null,
  bestTimeMs: null,
  dailyResults: [] as readonly KlondikeDailyResult[],
  startedRunIds: [] as readonly string[],
  wonRunIds: [] as readonly string[],
};

export const useKlondikeStatsStore = create<KlondikeStatsState>()(
  persist(
    (set) => ({
      ...EMPTY,
      recordStart: (runId) =>
    ...
    {
      name: KLONDIKE_STATS_STORAGE_KEY,
      version: 1,
      partialize: (state) => ({
...
function upsertDaily(
  current: readonly KlondikeDailyResult[],
  result: KlondikeDailyResult,
): KlondikeDailyResult[] {
...
export function dailyResultFor(
  results: readonly KlondikeDailyResult[],
  key: string,
): KlondikeDailyResult | null {
  return results.find((result) => result.key === key) ?? null;
}

/** Includes today when complete; otherwise a current streak may end yesterday. */
export function dailyStreak(results: readonly KlondikeDailyResult[], todayKey: string): number {
