...
import { dailyResultFor, dailyStreak, useSpiderStatsStore } from './spiderStats';

beforeEach(() => {
  useSpiderStatsStore.getState().reset();
});

describe('Spider local stats', () => {
  it('deduplicates starts and wins by run id', () => {
    const stats = useSpiderStatsStore.getState();
    ...
    expect(useSpiderStatsStore.getState()).toMatchObject({
      dealsStarted: 1,
      wins: 1,
      bestMoves: 120,
      bestTimeMs: 90_000,
  ...
  it('keeps independent best moves/time for a replayed daily table', () => {
    ...
    expect(dailyResultFor(useSpiderStatsStore.getState().dailyResults, '2026-08-24')).toEqual({
      key: '2026-08-24',
      bestMoves: 110,
      bestTimeMs: 70_000,
      completedAtMs: 2,
