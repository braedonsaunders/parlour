...
export function FreecellFxLayer({
  fx,
  fxKey,
  rootRef,
  reduced,
  ...
  useTableAudio(fx, fxKey, 'freecell');
  useSolitaireNarration('freecell', fx, fxKey);
  ...
  const won = fx.some((event) => event.kind === 'freecell.win');
  ...
  return (
    <div className={tableStyles.fxLayer} aria-hidden="true">
      ...
      {won ? (
        <div className={styles.winMoment} data-fx-cue="freecell-win" aria-hidden="true">
          <span>♠</span>
          <span>♥</span>
...
function planFlights(fx: readonly FxEvent[]): Flight[] {
  return fx.flatMap((event, eventIndex) => {
    ...
    if (event.kind !== 'freecell.cards-move' || !Array.isArray(payload.cards)) return [];
