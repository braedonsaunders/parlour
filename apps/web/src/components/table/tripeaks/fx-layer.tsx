...
export function TripeaksFxLayer({
  fx,
  fxKey,
  rootRef,
  reduced,
  ...
  useTableAudio(fx, fxKey, 'tripeaks');
  useSolitaireNarration('tripeaks', fx, fxKey);
  ...
  const cleared = fx.some((event) => event.kind === 'tripeaks.clear');
  ...
  return (
    <div className={tableStyles.fxLayer} aria-hidden="true">
      ...
      {cleared ? (
        <div className={styles.winMoment} data-fx-cue="tripeaks-clear" aria-hidden="true">
          <span>♠</span>
          <span>♥</span>
          <strong>Clear!</strong>
          <span>♦</span>
...
function planFlights(fx: readonly FxEvent[]): Flight[] {
  return fx.flatMap((event, eventIndex) => {
    ...
    if (!payload) return [];
    if (event.kind === 'card.fly' || event.kind === 'tripeaks.stock-flip') {
      ...
      return [
        {
          ...
          from: typeof from === 'string' ? from : 'stock',
          to,
    ...
    if (event.kind !== 'tripeaks.play' || !Array.isArray(payload.cards)) return [];
