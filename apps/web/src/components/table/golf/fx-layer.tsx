...
export function GolfFxLayer({
  ...
  return (
    <div className={tableStyles.fxLayer} aria-live="polite">
      ...
      {won ? (
        <div className={styles.winMoment} data-fx-cue="golf-win" aria-hidden="true">
          <span>♠</span>
          <span>♥</span>
          <strong>Clear!</strong>
          <span>♦</span>
...
function planFlights(fx: readonly FxEvent[]): Flight[] {
  return fx.flatMap((event, eventIndex) => {
    ...
    if (event.kind !== 'golf.cards-move' || !Array.isArray(payload.cards)) return [];
