...
export function PyramidFxLayer({
  fx,
  fxKey,
  rootRef,
  reduced,
  ...
  useTableAudio(fx, fxKey, 'pyramid');
  useSolitaireNarration('pyramid', fx, fxKey);
  ...
  const won = fx.some((event) => event.kind === 'pyramid.win' || event.kind === 'pyramid.hole-out');
  ...
  return (
    <div className={tableStyles.fxLayer} aria-hidden="true">
      ...
      {won ? (
        <div className={styles.winMoment} data-fx-cue="pyramid-win" aria-hidden="true">
          <span>♠</span>
          <span>♥</span>
          <strong>Clear!</strong>
          <span>♦</span>
...
function planFlights(fx: readonly FxEvent[]): Flight[] {
  return fx.flatMap((event, eventIndex) => {
    ...
    if (!payload) return [];
    if (event.kind === 'card.fly' || event.kind === 'card.draw' || event.kind === 'card.discard') {
      const card = payload.card;
      const from = payload.from ?? (event.kind === 'card.discard' ? payload.to : undefined);
      const to = payload.to;
      if (typeof card !== 'string' || typeof to !== 'string') return [];
      const origin = typeof from === 'string' ? from : 'stock';
      return [
        {
