...
export function SpiderFxLayer({
  ...
  useTableAudio(fx, fxKey, 'spider');
  useSolitaireNarration('spider', fx, fxKey);
  ...
  const won = fx.some((event) => event.kind === 'spider.win');
  ...
  return (
    <div className={tableStyles.fxLayer} aria-hidden="true">
      ...
      {won ? (
        <div className={styles.winMoment} data-fx-cue="spider-win" aria-hidden="true">
          <span>♠</span>
          <span>♥</span>
...
function planFlights(fx: readonly FxEvent[]): Flight[] {
  return fx.flatMap((event, eventIndex) => {
    ...
    if (
      (event.kind !== 'spider.cards-move' && event.kind !== 'spider.stock-deal') ||
      !Array.isArray(payload.cards)
    ) {
      return [];
    }
    if (event.kind === 'spider.stock-deal') return [];
